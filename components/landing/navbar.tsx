        ...
        {isModalOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-4xl max-h-[92vh] bg-white rounded-3xl border border-slate-200 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close button - inside header, not absolute */}
                ...
                <AnimatePresence mode="wait">
                  {modalStep === "schedule" ? (
                    ...
                    >
                      ...
                      {/* Body: Date + Time side by side */}
                      <div className="flex-1 grid md:grid-cols-2 gap-0 divide-x divide-slate-100 overflow-hidden">
                        {/* Date Selector */}
                        <div className="p-5 overflow-y-auto">
                          <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-50 text-[#07254B] border border-blue-100 text-xs">1</span>
                            Select a Date
                          </h3>
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <div className="flex gap-1.5">
                                <button onClick={prevMonth} className="w-8 h-8 rounded-full flex items-center justify-center border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
                                  <ChevronLeft className="w-4 h-4" />
                                </button>
                                <button onClick={nextMonth} className="w-8 h-8 rounded-full flex items-center justify-center border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors">
                            ...
                            <div className="grid grid-cols-7 gap-y-0.5 gap-x-0">
                              {calendarDays.map((day, idx) => {
                                if (!day) return <div key={`empty-${idx}`} className="flex justify-center"><div className="w-9 h-9" /></div>;
                        ...
                        {/* Time Selector */}
                        <div className={`p-5 overflow-y-auto transition-all duration-500 ${selectedDate !== null ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
                          <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                            <span className="flex items-center justify-center w-6 h-6 rounded-full bg-blue-50 text-[#07254B] border border-blue-100 text-xs">2</span>
                            Choose a Time Slot
                          </h3>
                            ...
                            {timeSlots.map((time, idx) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`relative py-2.5 rounded-xl text-xs font-bold border transition-all duration-200 ${!isAvailable
                                    ? "bg-slate-50 border-slate-100 text-slate-300 cursor-not-allowed"
                                    : isSelected
                                      ? "bg-[#07254B] border-[#07254B] text-white shadow-md scale-105"
                                      : "bg-white border-slate-200 text-slate-600 hover:border-[#07254B]/30 hover:bg-slate-50 hover:text-[#07254B]"
                                    }`}
                                >
                                  {time}
                                  {!isAvailable && <span className="absolute -top-2 -right-2 bg-slate-100 text-slate-400 text-[9px] px-1.5 py-0.5 rounded-full border border-slate-200">Full</span>}
                                </button>
                              )
                            })}
                          </div>
                      ...
                      <div className="px-5 py-3 border-t border-slate-100 bg-white">
                        <Button
                          ...
                          className="w-full bg-[#07254B] hover:bg-[#07254B]/90 text-white rounded-xl h-11 text-sm font-bold group shadow-sm disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                        >
                          {selectedDate === null || selectedTime === null
                            ? "Select a date & time to book an agent"
                            : <><span>Confirm & Book Agent</span><ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" /></>}
                        </Button>
                      </div>
                    </motion.div>
                  ) : (
                      ...
                      <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
                        ...
                        {selectedDate !== null && selectedTime && (
                          <div className="flex-1 flex justify-center">
                            <div className="flex items-center gap-1.5 text-xs font-semibold bg-[#07254B]/5 border border-[#07254B]/20 text-[#07254B] px-3 py-1.5 rounded-full">
                        ...
                        <button
                          onClick={closeModal}
                          className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-900 transition-colors shrink-0 ml-auto"
                        >
